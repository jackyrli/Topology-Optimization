...
classdef VM_structure < handle
    ...
    methods
        ...
        function vdata = parse_input(obj, v_input)
            %round the first 12 elements to nearest integer.
            v_input(1:12) = round(v_input(1:12));
...
%                 VonMisesStress = avgvM;
                ...
                %for recording only
                cost1 = abs(stiffnesspervolume)/1e7; %normalized to up to 3.3
                disp(['cost is : ',num2str(cost1)])
                vdata = [v1,v2,v3];
                sym_cost = obj.symmetry(vdata);
                writematrix([vdata'; cost1; sym_cost],['sim_results/sim',num2str(obj.numcalls)])
                
                % change for multiobjective
                cost = [cost1, sym_cost];
        ...
        function sym_cost = symmetry(obj,vdata)
            try
                displ = -1e-3;
                g = 9.8;
                rho0 = obj.rho0;
...
%% parse vdata
                ...
                % end rotate clockwise 90 degrees.
                v1 = vdata90(1:25);
                v2 = vdata90(26:75);
                v3 = vdata90(76:100);
%% end of parsing
...
%                             ygp = gcgp(2,gp); % y coordinate of Gaussian point
                ...
                %for recording only
                sym_cost = abs(stiffnesspervolume)/1e7; %normalized to up to 3.3
                disp(['cost of symmetry is : ',num2str(sym_cost)])
                %end of edit for cost definition and opt
            catch ME
                ...
                ME.stack(1).name, ME.stack(1).line, ME.message);
                fprintf(1, '%s\n', errorMessage);
                sym_cost = 0; %for unexpected errors
                %writematrix([vdata'],['error',num2str(obj.error_caught)])
            end
        end    
