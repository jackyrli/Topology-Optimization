...
%% end of parsing
                ...
                for e = 1:ne % loop over elements
                    ...
                    if nint > 0 % active node
                        for gp = 1:4 % summation over Gaussian points
                            ...
                            bgp = [0; -rho0*g]; %#ok<*PROPLC> % input body force
                            if norm(bgp) > 0
                                Fbefull = Fbefull + we*Ne(2*gp-1:2*gp,:)'*bgp*obj.h^2/4;
                            end
                        end
                    ...
                    elseif nint == 4 % full element
                        ...
                        if Nn > 0 % Neumann partial element
                            [Ke,Fbe,Fte,typen] = obj.NeumannBC(XN(Nn),YN(Nn),AN(Nn),BN(Nn),thetaN(Nn),obj.h,e,en,gcen,xmin,ymin,xmax,ymax,nint,next,intn,extn,typen,D,rho0,g,Kefull,Fbefull,maxit,epsilon,displayerror); % Neumann partial element arrays
                ...
                if ~isempty(Dfulln) % Dirichlet full element nodes are non-empty
                    for i = 1:length(Dfulln) % loop over Dirichlet full element nodes
                        ...
                        if n == nnx % bottom right
                            Dc = 1:2; % input Dirichlet BC coordinates
                            ubar = [0; 0]; % input Dirichlet BC
                        elseif mod(n,nnx) == 1 % left
                            Dc = 1; % input Dirichlet BC coordinates
                            ubar = [displ; 0]; % input Dirichlet BC
                        elseif mod(n,nnx) == 0 % right (symmetry line)
                            Dc = 1; % input Dirichlet BC coordinates
                            ubar = [0; 0]; % input Dirichlet BC
                ...
                F1top = sum(r1(nnx*(nny-1)+1:nnx*nny));
                F1left = sum(r1(linspace(1,nnx*(nny-1)+1,nnx)));
                F1right = sum(r1(linspace(nnx,nnx*nny,nnx)));
                F1row = sum(r1(nnx+1:2*nnx));
                F1column = sum(r1(linspace(2,nnx*(nny-1)+2,nnx)));
                F1total = sum(r1,'omitnan');
                F2bottom = sum(r2(1:nnx));
                F2top = sum(r2(nnx*(nny-1)+1:nnx*nny));
                F2left = sum(r2(linspace(1,nnx*(nny-1)+1,nnx)));
                F2right = sum(r2(linspace(nnx,nnx*nny,nnx)));
                F2row = sum(r2(nnx+1:2*nnx));
                F2column = sum(r2(linspace(2,nnx*(nny-1)+2,nnx)));
                F2total = sum(r2,'omitnan');

                % Reaction surface forces
                %disp('Reaction forces:')
                %R1top = F1top-F1row/2;
                %R1bottom = F1bottom-F1row/2;
                R1left = F1left-F1column/2;
                R1right = F1right-F1column/2;
                R2top = F2top-F2row/2;
                R2bottom = F2bottom-F2row/2;
                %R2left = F2left-F2column/2;
                %R2right = F2right-F2column/2;
                FEMFb1=R1left+R1right;
                FEMFb2=R2top+R2bottom;
                ...
                % Stiffness
                stiffnesspervolume1 = (abs(R1left)+abs(R1right))/2/abs(2*displ)/Vmatl;
                %stiffnesspervolume2 = (abs(R2top)+abs(R2bottom))/2/abs(displ)/Vmatl
                ...
                xc = repmat(linspace(Xmin+obj.h/2,obj.Xmax-obj.h/2,obj.nex),[obj.ney 1]);
                yc = repmat(linspace(Ymin+obj.h/2,obj.Ymax-obj.h/2,obj.ney)',[1 obj.nex]);
                u1n = reshape(u1,nnx,nny)';
                u2n = reshape(u2,nnx,nny)';
                r1n = reshape(r1,nnx,nny)';
                r2n = reshape(r2,nnx,nny)';
                sn = reshape(sn,nnx,nny)';
                sigma11c = reshape(sigma11,obj.nex,obj.ney)';
                sigma22c = reshape(sigma22,obj.nex,obj.ney)';
                sigma12c = reshape(sigma12,obj.nex,obj.ney)';
                sigmavMc = reshape(sigmavM,obj.nex,obj.ney)';
                sigma11c([1:hidenodes obj.ney-hidenodes+1:obj.ney],[1:hidenodes obj.nex-hidenodes+1:obj.nex]) = NaN;
                sigma22c([1:hidenodes obj.ney-hidenodes+1:obj.ney],[1:hidenodes obj.nex-hidenodes+1:obj.nex]) = NaN;
                sigma12c([1:hidenodes obj.ney-hidenodes+1:obj.ney],[1:hidenodes obj.nex-hidenodes+1:obj.nex]) = NaN;
                sigmavMc([1:hidenodes obj.ney-hidenodes+1:obj.ney],[1:hidenodes obj.nex-hidenodes+1:obj.nex]) = NaN;
                
%% START DOING ROTATING CALCULATION
                ...
                %for recording only
                cost1 = abs(stiffnesspervolume1)/1e7; %normalized to up to 3.3
                cost2 = abs(stiffnesspervolume2)/1e7; %normalized as well
                disp(['cost is : ',num2str(cost1)])
                vdata = [v1,v2,v3];
                
                writematrix([vdata'; cost1; cost2],['sim_results/sim',num2str(obj.numcalls)])
        ...
        function [Ke,Fbe,Fte,typen] = NeumannBC(obj,XN,YN,AN,BN,thetaN,h,e,en,gcen,xmin,ymin,xmax,ymax,nint,next,intn,extn,typen,D,rho0,g,Kefull,Fbefull,maxit,epsilon,displayerror) %#ok<INUSD> 
