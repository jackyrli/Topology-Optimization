...
classdef VM_structure < handle
    ...
    methods
        ...
        %parse function for 25 + num_defects * 3
        function vdata = parse_input(obj,v_input)
            %disp(v_input(1:25))
            % start parse input from heavyside function
            tf_stats = v_input(1:25);
            tf_stats = 1./(1+exp(-(20*tf_stats-10)));
            %tf_stats = round(tf_stats);
            % end heavyside parsing
            % start defects testing and parsing
            if sum(round(tf_stats)) ~= obj.num_defects
                disp('from matlab line 55 solid area')
                disp((4-sum(v_input(26:25+12).*v_input(38:37+12))*pi)/4)
                vdata = sum(round(tf_stats)); % indicating violation of constraint
                
            else
                vdata = zeros(1,100);
                vdata(1:25) = round(tf_stats);
                count_defects = 1;
                for i = 1:25
                    if tf_stats(i) >= 0.5 %defect in place    
            ...
            % end defects testing and parsing
        end
...
%                 VonMisesStress = avgvM;
                ...
                %for recording only
                cost = -abs(stiffnesspervolume)/1e7;
                disp(['cost is : ',num2str(cost)])
                vdata = [v1,v2,v3];
                writematrix([vdata'; cost],['sim',num2str(obj.numcalls)])
